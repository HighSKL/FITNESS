...
export const patch = async (path: string, data: Object) => (await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/${path}`, {cache: 'no-store', method: "PATCH", body: JSON.stringify(data)}).then(res=>res.json()))
